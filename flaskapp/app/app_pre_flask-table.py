...
def search():
    ...
    cursor = mysql.get_db().cursor()
    cursor.execute("SELECT school_name,district_name, \
    num_students,school_rank,student_ratio,test_scores,address from \
    flaskDB.schools where state='"+ state +"' AND city='"+ city +"'")
